...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Fucoin: starting difficulty is 1 / 2^12
...
{
    // Fucoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Fucoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 2.0 * 24 * 60 * 60; // Fucoin: 3.5 days
static const int64 nTargetSpacing = 2.0 * 60; // Fucoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Fucoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Fucoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Fucoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "yo yo yolo time boyo";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1389300694;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 313378088;
        ...
        assert(block.hashMerkleRoot == uint256("0x64a31cfe45bb53113c187221bb2949031f6d3249c61f46f35f41f87608cb02ac"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Fucoin: increase each by adding 2 to bitcoin's value.
...
void static FucoinMiner(CWallet *pwallet)
{
    printf("FucoinMiner started\n");
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&FucoinMiner, pwallet));
}
